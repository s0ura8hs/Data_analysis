...
area_ranking = df.groupby('location').agg({
    'score': 'mean',
    'votes': 'sum'
}).sort_values(by='score', ascending=False)

print(area_ranking.head(10))

cuisine_ranking = df.groupby('cuisines').agg({
    'score': 'mean',
    'votes': 'sum'
}).sort_values(by='score', ascending=False)

print(cuisine_ranking.head(10))

top_restaurants = df.sort_values(by='score', ascending=False)

print(top_restaurants[['name', 'location', 'cuisines', 'rate', 'votes', 'score']].head(10))

def rank_by_column(column):
    ranking = df.groupby(column).agg({
        'score': 'mean',
        'votes': 'sum'
    }).sort_values(by='score', ascending=False)
...
# Examples
print(rank_by_column('rest_type'))
print(rank_by_column('online_order'))

top_areas = area_ranking.head(10)
...
sns.barplot(x=top_areas.index, y=top_areas['score'])
plt.xticks(rotation=45)
plt.title("Top Areas by Restaurant Quality")
plt.show()
